...
export function Login() {
  ...
  useEffect(() => {
    if (isAuthenticated && !authLoading) {
      logger.info('Usuário já autenticado, redirecionando para dashboard', {
        tag: 'Login'
      });
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      addLogEntry('auth', 'Tentativa de login iniciada', {
        email
      });
      ...
      if (result.success) {
        ...
        addLogEntry('auth', 'Login bem-sucedido, redirecionando para dashboard', {
          email
        });
        ...
        addLogEntry('error', 'Falha no login', {
      ...
      addLogEntry('error', 'Exceção durante login', {
        error,
        email
      });
      toast({
  ...
  if (authLoading) {
    return <Card className="w-full max-w-md animate-fade-in">
      ...
      <form onSubmit={handleSubmit}>
        <CardContent className="space-y-4">
          <div className="space-y-2">
            ...
            <Input id="email" type="email" placeholder="seu@email.com" value={email} onChange={e => setEmail(e.target.value)} required className="w-full" disabled={isLoading} autoComplete="email" />
          </div>
          <div className="space-y-2">
            ...
            <div className="relative">
              <Input id="password" type={showPassword ? "text" : "password"} placeholder="••••••••" value={password} onChange={e => setPassword(e.target.value)} required className="w-full pr-10" disabled={isLoading} autoComplete="current-password" />
              <button type="button" className="absolute inset-y-0 right-0 pr-3 flex items-center text-sm leading-5" onClick={togglePasswordVisibility} disabled={isLoading}>
        ...
        <CardFooter className="flex flex-col gap-4">
          <Button type="submit" disabled={isLoading} className="w-full bg-quiz hover:bg-quiz-dark text-slate-50">
