...
export function QuizConfigurationPanel({
  ...
  const handleSeedQuiz = async () => {
    ...
    try {
      logger.info('Inicializando questionário', {
        tag: 'Quiz'
      });
      ...
      if (error) {
        ...
        logger.info('Questionário inicializado com sucesso', {
          tag: 'Quiz'
        });
  ...
  return <div className="w-full max-w-4xl mx-auto">
      <div className="mb-8 text-center">
        ...
        {loadError && <div className="bg-red-50 border border-red-200 text-red-700 px-4 py-3 rounded-md mb-6">
            <p className="font-medium">Erro ao carregar dados</p>
            <p className="text-sm">{loadError}</p>
            <Button onClick={onRefresh} className="mt-2 bg-red-600 hover:bg-red-700">
              Tentar novamente
            </Button>
          </div>}
        
        {!isLoading && modules.length === 0 && !loadError && <div className="bg-yellow-50 border border-yellow-200 text-yellow-700 px-4 py-3 rounded-md mb-6">
      ...
      {isLoading ? <div className="flex flex-col items-center justify-center p-8">
          ...
          <div className="grid md:grid-cols-3 gap-8">
            <div>
              ...
              <Select value={selectedModuleId || ""} onValueChange={value => setSelectedModuleId(value)}>
                ...
                <SelectContent>
                  {modules.map(module => <SelectItem key={module.id} value={getSelectValue(module.id, `module-${module.order_number}`)}>
            ...
            <div>
              ...
              <Select value={selectedQuestionId || ""} onValueChange={value => setSelectedQuestionId(value)} disabled={!selectedModuleId}>
                ...
                <SelectContent>
                  {filteredQuestions.map(question => <SelectItem key={question.id} value={getSelectValue(question.id, `question-${question.order_number}`)}>
                      {/* Compatibilidade com ambos os tipos de pergunta */}
                      {question.question_number || question.order_number}. {truncateText(question.question_text || question.text, 30)}
                    </SelectItem>)}
                </SelectContent>
              </Select>
            </div>
