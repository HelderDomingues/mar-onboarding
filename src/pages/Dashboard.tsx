...
const Dashboard = () => {
  ...
  return <div className="min-h-screen flex flex-col bg-gray-50">
      ...
      <main className="flex-1 container max-w-5xl py-8 px-4">
        ...
        {loadError && <div className="mb-6 p-4 bg-destructive/10 text-destructive rounded-lg">
            <p>{loadError}</p>
          </div>}

        {isAdmin && <Card className="w-full mb-6 border-violet-500 border-2 bg-violet-100">
            <CardHeader className="pb-3 rounded-sm">
                ...
                <Tabs defaultValue="quiz">
                  ...
                  <TabsContent value="quiz" className="space-y-4">
                    <div className="flex flex-col gap-3">
                      <Button onClick={navigateToQuizAdmin} className="justify-start bg-white border border-orange-200 text-orange-700 hover:bg-orange-50 hover:text-orange-800">
                        <Settings className="mr-2 h-4 w-4" />
                        Modo administrador do questionário
                        <ChevronRight className="ml-auto h-4 w-4" />
                      </Button>
                      
                      <Button variant="outline" onClick={handleSeedData} disabled={isSeeding} className="justify-start border-orange-200 text-orange-700 hover:bg-orange-50 hover:text-orange-800">
                        {isSeeding ? <>
                  ...
                  <TabsContent value="users" className="bg-white p-4 rounded-md border border-orange-200">
                    ...
                    <div className="flex items-center mb-4 gap-2">
                      <Input placeholder="Buscar usuário..." onChange={e => setSearchTerm(e.target.value)} className="max-w-sm" value={searchTerm} />
                      <Button variant="outline" className="border-orange-200 text-orange-700 hover:bg-orange-50" onClick={loadUsers}>
                        <Search className="h-4 w-4 mr-2" /> Buscar
                      </Button>
                    </div>
                    
                    {isLoadingUsers ? <div className="py-8 flex justify-center">
                        <div className="animate-spin rounded-full h-8 w-8 border-b-2 border-orange-500"></div>
                      </div> : users.length > 0 ? <div className="rounded-md border overflow-hidden">
                        <Table>
                          <TableHeader>
                            <TableRow>
                          ...
                          <TableBody>
                            {users.filter(user => searchTerm ? user.email.toLowerCase().includes(searchTerm.toLowerCase()) : true).map(user => <TableRow key={user.id}>
                                  <TableCell>{user.email}</TableCell>
                                  <TableCell>{user.createdAt}</TableCell>
                                  <TableCell>{user.lastLogin}</TableCell>
                                  <TableCell>
                                    <span className={`px-2 py-1 rounded-full text-xs ${user.role === 'admin' ? 'bg-orange-100 text-orange-800' : 'bg-blue-100 text-blue-800'}`}>
                                      {user.role || 'user'}
                                    </span>
                                  </TableCell>
                                  <TableCell>
                                    <Button variant="ghost" size="sm" onClick={() => handleToggleAdmin(user.id, user.role)} className={user.role === 'admin' ? 'text-red-600 hover:text-red-700 hover:bg-red-50' : 'text-green-600 hover:text-green-700 hover:bg-green-50'}>
                  ...
                  <TabsContent value="data" className="bg-white p-4 rounded-md border border-orange-200">
                    ...
                    <Button variant="outline" className="border-orange-200 text-orange-700 hover:bg-orange-50 hover:text-orange-800" onClick={() => toast({
        ...
        <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
          <Card className="hover:shadow-md transition-shadow">
            ...
            <CardContent>
              {submission ? submission.completed ? <div className="flex items-center gap-2 text-sm text-green-600 mb-2">
            ...
            <CardFooter>
              <Button className="w-full bg-quiz hover:bg-quiz-dark" onClick={() => navigate("/quiz")}>
          ...
          <Card className="hover:shadow-md transition-shadow">
            ...
            <CardFooter>
              <Button variant="outline" className="w-full border-quiz text-quiz hover:bg-quiz/10" disabled={!submission?.completed}>
                Ver Resultados
              </Button>
            </CardFooter>
          </Card>
          ...
          <Card className="hover:shadow-md transition-shadow">
            ...
            <CardFooter>
              <Button variant="outline" className="w-full border-quiz text-quiz hover:bg-quiz/10">
                Acessar Materiais
              </Button>
            </CardFooter>
          </Card>
